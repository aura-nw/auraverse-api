...
export default class AccountService extends Service {
		...
		this.parseServiceSchema({
			...
			actions: {
				...
				verifyCodeIdOwnership: {
					...
					async handler(
						ctx: Context<VerifyCodeIdOwnershipRequest>,
					): Promise<ResponseDto> {
	...
	public async changePassword(ctx: Context<ChangePasswordRequest>): Promise<ResponseDto> {
		...
		await this.accountMixin.update(
			{ id: account.id },
			{ password: bcrypt.hashSync(ctx.params.newPassword!, 8) },
		);
	...
	public async changeEmail(ctx: Context<ChangeEmailRequest>): Promise<ResponseDto> {
		...
		} else if (!ctx.params.password) {
			return ResponseDto.response(ErrorMap.EMAIL_VALID);
		}
	...
	public async verifyCodeIdOwnership(
		ctx: Context<VerifyCodeIdOwnershipRequest>,
	): Promise<ResponseDto> {
		const [codeIdOnChain, codeIdAurascan, codeIdDb]: [any, any, CodeId] = await Promise.all([
			this.callApiFromDomain(
				[process.env.LCD],
				`${ApiQuery.GET_DATA_CODE_ID}${ctx.params.codeId}`,
			),
			this.callApiFromDomain(
				[process.env.AURASCAN_API],
				`${ApiQuery.GET_CODE_ID_VERIFICATION}${ctx.params.codeId}`,
			),
			this.codeIdMixin.findOne({ code_id: ctx.params.codeId }),
		]);
		if (!codeIdOnChain.code_info) {
			return ResponseDto.response(ErrorMap.E007, { request: ctx.params });
		}
		if (codeIdAurascan.data.contract_verification === ContractVerification.UNVERIFIED) {
			return ResponseDto.response(ErrorMap.E008, { request: ctx.params });
		}
		if (codeIdDb) {
			return ResponseDto.response(ErrorMap.E009, { request: ctx.params });
		}

		if (!ctx.params.accountId || ctx.params.accountId === 0) {
		...
		return ResponseDto.response(ErrorMap.VERIFY_CODE_ID_OWNERSHIP, {
			codeId: ctx.params.codeId,
		});
	}
}
