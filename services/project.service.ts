...
export default class ProjectService extends Service {
	...
	public async createProject(ctx: Context<CreateProjectRequest>): Promise<ResponseDto> {
		...
		if (ctx.params.codeIds && ctx.params.codeIds!.length > 0) {
			const codeIds: CodeId[] = await this.codeIdMixin.find({
				code_id: [...ctx.params.codeIds!],
			});
	...
	public async updateProject(ctx: Context<UpdateProjectRequest>): Promise<ResponseDto> {
		...
		if (ctx.params.codeIds && ctx.params.codeIds!.length > 0) {
			...
			const notOwnedCodeIds: CodeId[] = codeIds.filter(
				(codeId: CodeId) => codeId.accountId !== ctx.params.accountId,
			);
			if (notOwnedCodeIds.length > 0) {
				return ResponseDto.response(ErrorMap.E019, { codeIds: notOwnedCodeIds });
			}

			const missingCodeIds: (number | undefined)[] = _.difference(
				ctx.params.codeIds!,
				codeIds.map((c: CodeId) => c.codeId),
			);
			if (missingCodeIds.length > 0) {
			...
			await this.projectMixin.update(
			...
			const requestProject = _.omit(ctx.params, ['codeIds', 'categories', 'projectId']);
		...
		if (projects.length > 0) {
			...
			projects.map((project: Project) => {
				...
				if (today.diff(projectCreatedDate, 'day') >= 30) {
					listUpdates.push(
						this.projectMixin.update({ id: project.id }, { is_new: false }),
					);
				}
			});
