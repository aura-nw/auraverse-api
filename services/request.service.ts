...
export default class RequestService extends Service {
	...
	public constructor(public broker: ServiceBroker) {
		...
		this.parseServiceSchema({
			...
			mixins: [queueService(queueConfig.redis, queueConfig.opts), this.callApiMixin],
			actions: {
				/**
				 * View your requests
				 *
			...
			queues: {
				'request.store-code-id': {
					...
					async process(job: Job) {
						job.progress(10);
						// @ts-ignore
						await this.uploadCodeIdMainnet(
							job.data.ids,
							job.data.email,
							job.data.requestId,
						);
	...
	public async requestStoreCodeId(ctx: Context<StoreCodeIdRequest>): Promise<ResponseDto> {
		...
		const missingCodeIds = _.difference(
			ctx.params.codeIds!,
			codeIds.map((c: CodeId) => c.codeId),
		);
		...
		switch (request.type) {
			case RequestType.CREATE:
				if (request.codeIds) {
					...
					const codeIdsCreate: CodeId[] = await this.codeIdMixin.find({
						code_id: idsCreate,
					});
				...
				listQueries.push(
					...[
						this.projectMixin.update(
							{ id: project.id },
							{ status: ProjectStatus.APPROVED },
						),
						this.requestMixin.update(
							{ id: request.id },
							{ status: ProjectStatus.APPROVED },
						),
			...
			case RequestType.UPDATE:
				...
				if (request.otherDocumentation) {
					project.otherDocumentation = request.otherDocumentation;
				}
				if (request.activeStatus) {
					project.activeStatus = request.activeStatus;
				}
				if (request.website) {
					project.website = request.website;
				}
				if (request.imageLink) {
					project.imageLink = request.imageLink;
				}
				if (request.categories) {
					project.categories = JSON.stringify(request.categories);
				}
				if (request.whitepaper) {
					project.whitepaper = request.whitepaper;
				}
				if (request.github) {
	...
	public async rejectRequest(ctx: Context<RejectRequest>): Promise<ResponseDto> {
		...
		if (request.status !== ProjectStatus.ERROR && request.status !== ProjectStatus.SUBMITTED) {
			return ResponseDto.response(ErrorMap.E021, { id: ctx.params.id });
		}
		...
		await Promise.all([
			...listQueries, // Update project status if needed
		...
		return ResponseDto.response(ErrorMap.REQUEST_REJECTED, { id: ctx.params.id });
	}
	...
	public async uploadCodeIdMainnet(ids: number[], email: string, requestId: number) {
		...
		try {
			for (const codeId of codeIds) {
				if (codeId.mainnetCodeId !== null && codeId.mainnetCodeId !== 0) {
					continue;
				}
