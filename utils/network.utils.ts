...
export class Network extends CosmWasmClient {
	...
	public handleError(error: Error) {
		if (error.message.search(AppConstants.INSUFFICIENT_FUNDS) > -1) {
			throw new Error(ErrorMap.E011.Message);
		}
		throw new Error(ErrorMap.E012.Message);
	}

	public async instantiateSmartcontract(
	...
	public async getUsedFee(
		...
		const gasEstimation = await this.simulate(signerAddress, messages, memo, this.account);
	...
	public async signDirect(
		...
		const stdSignature = encodeSecp256k1Signature(accountFromSigner.pubkey, signatureBytes);
